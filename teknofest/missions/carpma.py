...
v4 değişikliği:
  - Saldırı (STRIKING) sırasında hedef açısı STRIKE_MAX_ANGLE_DEG'i aşarsa
    ...
    bekliyor, sonra doğrudan MISS oluyordu. Artık bu durumda Task 3'ün
    ortak skid-steer dönüş komutuyla açı toparlanmaya çalışılıyor;
    toparlanırsa saldırıya kaldığı yerden devam ediliyor.
...
BACKOFF_CONFIRM_CAMERA_FRAMES=3
BACKOFF_MAX_HEADING_CORRECTION_RAD=0.18
BACKOFF_GPS_CONTRADICTION_DISTANCE_M=1.50
COOLDOWN_SEC=0.8
...
BASELINE_WINDOW=20
IMPACT_MIN_BASELINE_SAMPLES=10
...
class CarpmaGorevi:
    def __init__(self,node,mission_topics,target_class,
        ...
        self.backoff_camera_confirm_count=0
    ...
    def update_imu(self,ax,ay,az):
        ...
        if self.state!=CarpmaState.STRIKING:
            # Referans yalnız araç çarpma öncesi durmuşken CAMERA_CONFIRM
            # aşamasında toplanır. Arama/yaklaşma/backoff ivmelerini sakin-su
            # referansına katmak gerçek darbeyi maskeleyebilir.
            if self.state==CarpmaState.CAMERA_CONFIRM and self.current_speed==0.0:
                self.accel_baseline.append(mag)
            self.spike_count=0
            return
        if self.current_speed<IMPACT_MIN_FORWARD_SPEED:
            self.spike_count=0
            return
        target_is_fresh=(
    ...
    def _process_frame(self,detections,frame_id,now):
        ...
        if target is None:
            ...
            elif self.state==CarpmaState.BACKING_OFF:
                self.backoff_camera_confirm_count=0
            return False
        ...
        if self.state==CarpmaState.CAMERA_CONFIRM:
            ...
            if len(self.confirm_frame_ids)>=CAMERA_CONFIRM_FRAMES:
                self._finish_camera_confirmation(now)
            else:
                self.logger.info(
                    f"[ÇARPMA] Kamera onayı "
                    f"{len(self.confirm_frame_ids)}/{CAMERA_CONFIRM_FRAMES}."
                )
        elif self.state==CarpmaState.BACKING_OFF and self.backoff_start_distance is not None:
            distance_increase=(
                float(target['distance'])-self.backoff_start_distance
            )
            if distance_increase>=BACKOFF_REQUIRED_CAMERA_INCREASE_M:
                self.backoff_camera_confirm_count+=1
            else:
                self.backoff_camera_confirm_count=0
        return True

    def _finish_camera_confirmation(self,now):
        if len(self.accel_baseline)<IMPACT_MIN_BASELINE_SAMPLES:
            self.logger.warning(
                "[ÇARPMA] Kamera hazır ancak sakin IMU referansı bekleniyor.",
    ...
    def _strike(self,now):
        ...
        if abs(angle)>STRIKE_MAX_ANGLE_DEG:
            # Açı çok büyükse pes etmek yerine ortak skid-steer komutuyla
    ...
    def _backoff(self,now):
        ...
        if None not in (self.backoff_start_lat,self.backoff_start_lon,self.current_lat,self.current_lon,self.backoff_heading_deg):
            ...
            gps_contradicts=(
                moved>=BACKOFF_GPS_CONTRADICTION_DISTANCE_M
                and (
                    backward_progress<=0.0
                    or lateral>BACKOFF_MAX_LATERAL_M
                )
            )
        if gps_contradicts:
            self._miss("geri kaçış GPS'e göre yanlış yönde")
            return
        # Kısa 60 cm hareket normal GPS çözünürlüğünün altında kalabilir.
        # Bu nedenle üç farklı kamera karesindeki gerçek mesafe artışı ana
        # doğrulamadır; GPS ise ters/yanal büyük hareketi reddeden güvenlik
        # kontrolüdür.
        if self.backoff_camera_confirm_count>=BACKOFF_CONFIRM_CAMERA_FRAMES:
            stop_vehicle(self.topics.cmd_vel_pub,repeat_count=1); self.current_speed=0.0
            self.state=CarpmaState.COOLDOWN; self.cooldown_start_time=now
            self.logger.info(
                "[ÇARPMA] Geri kaçış kamera ile doğrulandı "
                f"(GPS doğrulaması={'var' if gps_far else 'belirsiz'})."
            )
            return
    ...
    def update(self,detections,frame_id=None):
        ...
        if self.state==CarpmaState.BACKING_OFF and frame_status is False:
            self._miss('geri kaçış sırasında hedef yeni kamera karesinde kayboldu'); return True
        ...
        if self.state==CarpmaState.CAMERA_CONFIRM: stop_vehicle(self.topics.cmd_vel_pub,repeat_count=1)
            ...
            if now-self.cooldown_start_time>=COOLDOWN_SEC:
                self.state=CarpmaState.CAMERA_CONFIRM; self.confirm_frame_ids.clear(); self.confirm_distances.clear(); self.confirm_angles.clear(); self.last_seen_time=None; self.camera_confirm_start_time=now; self.accel_baseline.clear()
    ...
    def prepare_for_collision(self):
        """Yaklaşmadan çarpmaya geçerken hareketli IMU geçmişini temizle."""
        self.accel_baseline.clear()
        self.spike_count=0
        self.current_speed=0.0
